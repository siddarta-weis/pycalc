...
class Calculadora(QWidget):
    ...
    def initialize_ui(self) -> None:
        ...
        self.add_button(QPushButton("%"), 2, 2, 1, 1, self.calc_percentage)
    ...
    def calc_percentage(self) -> None:
        if "÷" in self.line_edit.text():
            self.line_edit.setText(self.calc_percent_div())

        if "x" in self.line_edit.text():
            self.line_edit.setText(self.calc_percent_mult())

        if "-" in self.line_edit.text():
            self.line_edit.setText(self.calc_percent_sub())

        if "+" in self.line_edit.text():
            self.line_edit.setText(self.calc_percent_sum())
    
    def calc_percent_div(self) -> str:
        try:
            num, percent = self.line_edit.text().split("÷")
            num, percent = float(num), float(percent)
            result = num / (percent/100)
            return str(self.safe_to_int(result))
        except Exception as e:
            return "Value Error"
    
    def calc_percent_mult(self) -> str:
        try:
